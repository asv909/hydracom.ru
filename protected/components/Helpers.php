...
class Helpers 
{
     ...
     * @param string $login this is user name, taken from $POST['form']
     * @param string $secretword this is user password, taken from $POST['form']
     * @param string $salt this is user salt, taken from appropriate DB-record
     * @param string $suffix this is special string for added security, taken from variable $suffix that was set within class of appropriate model
    ...
    static function restrictNumberOfAttempts($numberOfAttempts = 5, $timeout = 600) 
    {
        ...
        if($attempt > $numberOfAttempts)
        {
            ...
            $elapsedTime = time()-$session->get('restrictTime', 0); 
