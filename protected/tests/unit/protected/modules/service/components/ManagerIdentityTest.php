...
{
    public $valid_data = array('username' => 'asv909', 
                               'password' => 'ErTrd-2007');
    ...
    {
        $this->_identity = new ManagerIdentity($this->valid_data['username'],
                                               $this->valid_data['password']);
        $manager = new Manager;
        $this->record = $manager->findByAttributes(array(
                            'login' => $this->_identity->username,
                        ));
        $this->_identity->setRecord($this->record);
        $this->_identity->setHash(Helpers::createHash($this->valid_data['username'],
                                                      $this->valid_data['password'],
                                                      $this->record->salt,
                                                      $this->suffix));
        // with valid data
        ...
        // test getID function
        $this->assertNotNull($this->_identity->getId());
